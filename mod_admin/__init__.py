...
mod_admin= Blueprint(name="mod_admin", import_name="mod_admin")
...
@mod_admin.route("/addstudent", methods=['post'])
def register():
   records= db.students
...
@mod_admin.route('/getmax', methods=['get'])
def getmaxroom():
   if db.rooms.count()==0:
      rnumber="1"
   else:
...
@mod_admin.route('/students',methods=['get'])
def list_student():
   records=db.students
...
#for displayin 
@mod_admin.route('/rooms',methods=['get'])
def list_rooms():
   records=db.rooms
...
#for adding a room
@mod_admin.route('/addrooms',methods=['post'])
def add_room():
   records= db.rooms
   file = request.files['photo']
   filename = file.filename
...
#for returning all the roomids to the add student table
@mod_admin.route('/roomid',methods=['get'])
def roomids():
   lst=[]
   records=db.rooms.distinct('rnumber')
   for i in records:
      lst.append(i)
   return render_template("roomids.html",lst=lst)
